...
function evaluate(total, numbers) {
  ...
  if (numbers.length == 2) {
    return total == numbers[0] + numbers[1] || total == numbers[0] * numbers[1] || total == Number(`${numbers[0]}${numbers[1]}`)
  }
  ...
  const concat = Number(`${numbers[0]}${numbers[1]}`)
  if (concat <= total) {
    if (evaluate(total, [concat, ...numbers.slice(2)])) {
      return true
    }
  }
