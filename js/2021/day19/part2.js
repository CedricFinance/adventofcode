...
aoc.run(function(input) {
    ...
    var max = 0
    for (let firstIndex = 0; firstIndex < scanners.length; firstIndex++) {
        const firstScanner = scanners[firstIndex];
        for (let secondIndex = 0; secondIndex < scanners.length; secondIndex++) {
            if (firstIndex == secondIndex) {
                continue
            }
            const secondScanner = scanners[secondIndex];
            max = Math.max(max, manhattanDistance(firstScanner.position, secondScanner.position))
        }
