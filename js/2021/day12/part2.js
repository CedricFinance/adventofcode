 ...
 * @param {boolean} canRevisit
 * @returns
 */
function countPaths(nodes, from, to, marked, canRevisit) {
    if (from == to) {
        return 1
    }

    if (marked.has(from)) {
        if (!canRevisit || from == "start" || from == "end") {
            return 0
        }
        canRevisit = false
    }
    ...
    for (const neighbor of node.neigbors) {
        paths += countPaths(nodes, neighbor, to, newMarked, canRevisit)
    }
...
aoc.run(function(input) {
    ...
    var result = countPaths(nodes, "start", "end", marked, true)
